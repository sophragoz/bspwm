...
if [ "$choice" -eq 1 ]; then
    echo "Installing....."
    sudo zypper in bspwm rofi polybar nitrogen lxappearance git zsh nemo picom
    cd bspwm
    cp -r .config ~/
    cp -r .xinitrc ~/
    ...
    sudo zypper in bspwm rofi polybar nitrogen lxappearance htop neovim git PrismLauncher steam telegram-desktop transmission-gtk openjdk21 krita vlc vscode nvidia nvidia-dkms librewolf eog nemo xournalpp intel-gpu-tools zsh pulseaudio pavucontrol libwebkit2gtk chronyd dhcpcd polkitd unzip psmisc lutris picom
    echo "Installing....."
    cd bspwm
    cp -r .config ~/
    cp -r .xinitrc ~/
